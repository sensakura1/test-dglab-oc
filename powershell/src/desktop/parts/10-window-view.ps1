        ...
        <Grid x:Name="SettingsPage" Grid.Row="2">
          ...
          <Border x:Name="CoyoteOutputPanel" Grid.Row="1" Grid.Column="1" Style="{StaticResource Card}" Margin="0,0,0,8">
            <StackPanel>
              ...
              <TextBlock Text="V3 原始强度为 0–200；Socket 模式下强度、通道和持续时间使用本软件设置，波形由 DG-Lab App 内设置决定。" Foreground="{StaticResource Muted}" TextWrapping="Wrap" Margin="0,4,0,6"/>
              <TextBlock x:Name="SocketOutputModeText" Text="Socket 波形由 App 决定" Foreground="#66B3FF" FontWeight="Bold" TextWrapping="Wrap" Visibility="Collapsed" Margin="0,0,0,14"/>
              <Grid>
                <Grid.ColumnDefinitions>
                  <ColumnDefinition Width="*"/>
                  <ColumnDefinition Width="*"/>
                ...
                <StackPanel Grid.Row="1" Margin="0,0,8,10">
                  <TextBlock Text="当前波形" Foreground="{StaticResource Muted}"/>
                  <Border Margin="0,4,0,0" Padding="10,7" Background="#141A24" CornerRadius="6">
                    <TextBlock x:Name="CurrentWaveformText" Text="不支持" Foreground="{StaticResource Ink}" TextWrapping="Wrap"/>
                  </Border>
                  <ComboBox x:Name="WaveformCombo" SelectedIndex="0" Visibility="Collapsed">
